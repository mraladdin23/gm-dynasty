  ...
  async fetch(req, env, ctx) {
    ...
    try {
      ...
      if (path === "/draft/diagnose" && req.method === "GET") {
        ...
        try {
          // 1. What is in draftWatchIndex for this user?
          const wiRes  = await fetch(`${FB}/draftWatchIndex/${username}.json?${auth}`);
          const wiData = wiRes.ok ? (await wiRes.json() || {}) : {};
          report.steps.watchIndex = { leagueCount: Object.keys(wiData).length, leagues: wiData };

          // 2. Simulate what client would write (from stored leagues)
          ...
          for (const [key, l] of Object.entries(leagues)) {
            ...
            if (season && !relevant.has(season) && !relevant.has(String(season))) { filtered.push({ key, reason: `season=${season} out of range` }); continue; }
            if (l.mostRecentSeason && String(l.season) !== String(l.mostRecentSeason)) { filtered.push({ key, reason: `stale dynasty (${l.season} vs mostRecent ${l.mostRecentSeason})` }); continue; }
            watching.push({ key, leagueId: String(l.leagueId || l.league_id), season: l.season, name: l.leagueName || l.name });
          }
          report.steps.leagueFilter = { total: Object.keys(leagues).length, watching: watching.length, filtered: filtered.length, watchingList: watching };

          // 3. Cross-check draftStatus + Sleeper for each watched league
          const now = Date.now();
          const statusResults = {};
          await Promise.all(watching.map(async w => {
            ...
            const active = Array.isArray(sleeperDrafts)
            ...
            statusResults[w.leagueId] = {
              name: w.name, season: w.season,
              workerStatus: sd?.status || "NOT IN draftStatus",
              ...
              sleeperDraft: active ? { status: active.status, draftId: active.draft_id, startTime: active.start_time } : null,
              inWatchIndex: !!wiData[w.leagueId],
              mismatch: active && sd?.status !== active.status ? `⚠️ Worker="${sd?.status || "none"}" Sleeper="${active.status}"` : null
            };
          }));
          report.steps.draftStatus = statusResults;

          const mismatches = Object.values(statusResults).filter(s => s.mismatch);
          const notInIndex = watching.filter(w => !wiData[w.leagueId]);
          if (!watching.length) report.summary = "No current-season Sleeper leagues — sync leagues in DLR first.";
          else if (notInIndex.length) report.summary = `⚠️ ${notInIndex.length} leagues not in draftWatchIndex — user needs to reload DLR to write their watchIndex.`;
          else if (mismatches.length) report.summary = `⚠️ ${mismatches.length} Worker/Sleeper mismatches — Worker cron hasn't processed these yet (check pending queue size).`;
          else report.summary = "All looks good.";
...
if (path === "/draft/test" && req.method === "GET") {
  ...
  try {
    ...
    // Step 1: read draftWatchIndex (same as cron)
    const wiRes  = await fetch(`${FB}/draftWatchIndex.json?${auth}`);
    const wiData = (await wiRes.json()) || {};
    const watchList = {};
    for (const [, ul] of Object.entries(wiData)) {
      if (ul && typeof ul === "object") {
        for (const [id, name] of Object.entries(ul)) {
          if (!watchList[id]) watchList[id] = { leagueName: name || id };
        }
      }
    }
    const allIds = Object.keys(watchList);
    log.push({ step: "watchIndex", userCount: Object.keys(wiData).length, leagueCount: allIds.length });

    // Step 2: read existing draftStatus
    const statusRes = await fetch(`${FB}/draftStatus.json?${auth}`);
    const existing  = (await statusRes.json()) || {};
    log.push({ step: "draftStatus", status: statusRes.status, count: Object.keys(existing).length });

    // Step 3: figure out what toCheck looks like
    const now     = Date.now();
    const toCheck = [], toSkip = [];
    for (const id of allIds) {
      const prev = existing[id];
      if (!prev)                                        { toCheck.push(id); continue; }
      if (prev.status === "complete")                   { toSkip.push(id);  continue; }
      if (prev.status === "drafting" || prev.status === "paused") { toCheck.push(id); continue; }
      if (prev.status === "pre_draft" && prev.startTime) {
        if ((prev.startTime - now) > 15 * 60 * 1000)  continue;
        toCheck.push(id); continue;
      }
      toCheck.push(id);
    }
    log.push({ step: "filter", total: allIds.length, toCheck: toCheck.length, toSkip: toSkip.length });

    // Step 4: test just the FIRST toCheck league end-to-end
    if (toCheck.length > 0) {
      const testId  = toCheck[0];
      const meta    = watchList[testId] || {};
      const dr      = await fetch(`https://api.sleeper.app/v1/league/${testId}/drafts`);
      const drafts  = dr.ok ? await dr.json() : null;
      ...
      const draft   = Array.isArray(drafts)
        ...
        : null;
      log.push({ step: "firstLeague", leagueId: testId, sleeperStatus: dr.status, draftStatus: draft?.status || "none", draftId: draft?.draft_id });

      // Step 5: test the Firebase PATCH write
      const testWrite = { [testId]: { status: draft?.status || "pre_draft", picks_hash: "test", updatedAt: now } };
      const wr = await fetch(`${FB}/draftStatus.json?${auth}`, {
        method:  "PATCH",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify(testWrite)
      });
      log.push({ step: "patchWrite", status: wr.status, body: await wr.text() });
    }

    return new Response(JSON.stringify({ ok: true, log }, null, 2), { headers: corsHeaders() });
  } catch(e) {
    return new Response(JSON.stringify({ ok: false, error: e.message, stack: e.stack, log }, null, 2), 
...
if (path === "/draft/forcecheck" && req.method === "GET") {
  ...
  const r      = await fetch(`https://api.sleeper.app/v1/league/${id}/drafts`);
  const drafts = await r.json();
  const draft  = drafts.find(d => d.status === "drafting" || d.status === "paused");
  ...
  await fetch(`${FB}/draftStatus/${id}.json?${auth}`, {
    method:  "PUT",
    headers: { "Content-Type": "application/json" },
    body:    JSON.stringify(status)
  });

  return new Response(JSON.stringify({ ok: true, status }, null, 2), { headers: corsHeaders() });
}
...
async function runDraftWatcher(env) {
  ...
  const now     = Date.now();
  const urgent  = []; // live/paused/starting-soon — always check immediately
  const pending = []; // seeded or unknown — check a few per run
  const toSkip  = []; // complete — remove
  ...
  for (const id of allIds) {
    ...
    if (prev.status === "pre_draft") {
      ...
      const diff = prev.startTime - now;
      if (diff < 0)              { urgent.push(id);  continue; } // overdue
      if (diff < 15 * 60 * 1000){ urgent.push(id);  continue; } // <15 min
      if (diff < 60 * 60 * 1000){ pending.push(id); continue; } // <1 hour
      // >1 hour away — re-check every 15 min
      if (!prev.updatedAt || (now - prev.updatedAt) > 15 * 60 * 1000) {
        pending.push(id);
      }
      continue;
    }
  ...
  const writes  = {};
  const deletes = [...toSkip];

  const BATCH = 10;
  for (let i = 0; i < toCheck.length; i += BATCH) {
    const batch = toCheck.slice(i, i + BATCH);
    await Promise.allSettled(batch.map(async leagueId => {
      ...
      try {
        ...
        if (!draft) {
          // No active draft — mark as pre_draft with no startTime so we
          // re-check in 30 min rather than removing from watch list entirely
          writes[leagueId] = {
            status:     "pre_draft",
            picks_hash: "checked",
            startTime:  null,
        ...
        console.warn(`[DraftWatcher] ${leagueId}:`, e.message);
      }
    }));
  }
