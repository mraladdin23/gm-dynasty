...
// Apply any stored type overrides from this user's own leagueMeta
...
// ── FIX: Also check the shared salaryCap node for leagues where
//    this user has no local leagueTypeOverride (i.e. non-commissioners).
//    The commissioner writes leagueType:"salary" to gmd/salaryCap/{key}/settings
//    so all members can see the designation regardless of their own leagueMeta.
const salaryChecks = Object.keys(_allLeagues).map(async key => {
  if (_leagueMeta[key]?.leagueTypeOverride) return; // already set from local meta
  try {
    const settings = await GMDB.getSalarySettings(key);
    if (settings?.leagueType === "salary" && _allLeagues[key]) {
      _allLeagues[key].leagueType = "salary";
      console.log(`[DLR] Applied shared salary designation for league: ${key}`);
    }
  } catch(e) {
    // silently ignore — league just won't have shared override
  }
});
await Promise.all(salaryChecks);
...
el.innerHTML = `<span class="locker-stat"><strong>${stats.totalWins || 0}W–${stats.totalLosses || 0}L</strong></span> <span class="locker-stat-sep">·</span> <span class="locker-stat">${winPct} win rate</span> ${trophies ?`<span class="locker-stat-sep">·</span><span class="locker-stat">${trophies}</span>`: ""} <span class="locker-stat-sep">·</span> <span class="locker-stat">⭐ ${stats.dynastyScore || 0} pts</span>`;
}
...
container.innerHTML = connected.map(([platform, data]) => `<div class="platform-badge"> <span class="platform-badge-name">${_platformLabel(platform)}</span> <span class="platform-badge-user">${_platformUser(platform, data)}</span> </div>`).join(””);
}
...
try {
  ...
  // ── FIX: When commissioner sets salary cap designation, also write
  //    to the shared gmd/salaryCap/{leagueKey}/settings node so all
  //    league members can read it regardless of their own leagueMeta.
  if (typeOverride === "salary") {
    try {
      const existingSettings = await GMDB.getSalarySettings(leagueKey) || {};
      await GMDB.saveSalarySettings(leagueKey, {
        ...existingSettings,
        leagueType:         "salary",
        setByCommissioner:  true,
        setAt:              Date.now()
      });
      console.log(`[DLR] Wrote shared salary designation for league: ${leagueKey}`);
    } catch(e) {
      console.warn("[DLR] Failed to write shared salary designation:", e.message);
      // Non-fatal — local meta already saved above
    }
  } else if (typeOverride && typeOverride !== "salary") {
    // If overriding away from salary, clear the shared designation too
    try {
      const existingSettings = await GMDB.getSalarySettings(leagueKey) || {};
      if (existingSettings.leagueType === "salary") {
        await GMDB.saveSalarySettings(leagueKey, {
          ...existingSettings,
          leagueType: typeOverride,
          setByCommissioner: true,
          setAt: Date.now()
        });
      }
...
container.innerHTML = `<p class="preview-count">✓ ${entries.length} league${entries.length !== 1 ? "s" : ""} found</p> <ul class="preview-list"> ${entries.slice(0, 5).map(l =>`
<li>${_escHtml(l.leagueName)}
<span class="preview-season">${l.season}</span>
${l.isChampion ? “ 🏆” : “”}
</li>
`).join("")} ${entries.length > 5 ? `<li class="preview-more">+ ${entries.length - 5} more</li>`: ""} </ul>`;
}
...
const auctionOn   = meta.auctionEnabled || isSalary;
...
el.innerHTML = `<div class="detail-standings"> <div class="detail-standings-header"> <span>#</span><span>Team</span><span>W–L</span><span>PF</span> </div> ${standings.map(s =>`
