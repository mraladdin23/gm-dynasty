...
const DLRTournament = (() => {
  ...
  async function _renderView(tab) {
    ...
    try {
      ...
      // Boot the global draft ticker with the freshly-loaded tournament map
      DraftTicker.init(_tournaments);
    } catch(err) {
...
`}
  ...
  async function _renderAnalyticsDraft(tid, t, body) {
    ...
    try {
      ...
      // Refresh the global draft ticker with updated cache data
      DraftTicker.refresh(_tournaments);
    } catch(e) {
...
Write a 3\u20134 paragraph weekly recap in an engaging, sports-analyst style. Highlight the top scorer and give them a moment. Mention the closest game (nail-biter drama) and the biggest blowout. Take a gentle jab at the lowest scorer. Reference the score distribution to set the tone for the week \u2014 was it a high-scoring week, low-scoring, or all over the place? Keep it punchy and fun \u2014 like an ESPN segment. Use **bold** for team names and key scores. Keep total length under 350 words.`;
  ...
  // ── Global Draft Ticker ────────────────────────────────────────────────
  // Shows a pill in the nav with upcoming/live draft status across all
  // tournaments. Piggybacks on _draftCache already fetched by
  // _renderAnalyticsDraft and polls Sleeper every 30s for live status.
  // ──────────────────────────────────────────────────────────────────────
  const DraftTicker = (() => {
    let _tickerInterval    = null;
    let _tickerOpen        = false;
    let _checkedUpcoming   = {};  // leagueId → true (avoid repeat lookups)
    ...
    async function _getMySleeperUserId() {
      ...
      try {
        const snap = await GMD.child(`users/${_currentUsername}/platforms/sleeper/userId`).once("value");
        _mySleeperUserId = snap.val() || null;
      } catch(e) { /* non-fatal */ }
      return _mySleeperUserId;
    }

    // ── Gather live + upcoming draft items ────────────────
    async function _gatherItems(allTournaments) {
      const items = { live: [], upcoming: [] };
      if (!allTournaments) return items;

      const mySleeperUid = await _getMySleeperUserId();

      for (const [tid, t] of Object.entries(allTournaments)) {
        const batches = t.leagues || {};
        const isBatch = (v) => v && typeof v === "object" && v.leagues !== undefined;

        for (const [, batch] of Object.entries(batches)) {
          if (!isBatch(batch)) continue;
          const platform = batch.platform || "sleeper";
          const year     = batch.year;

          for (const [leagueId, l] of Object.entries(batch.leagues || {})) {
            const cacheKey = `${year}_${leagueId}`;
            const cached   = _draftCache?.byLeague?.[cacheKey];

            // ── Live draft ────────────────────────────────
            if (cached?.draft_status === "drafting") {
              let picksUntilMe = null;
              let onTheClock   = false;

              if (platform === "sleeper" && mySleeperUid) {
                try {
            ...
            // ── Upcoming draft (Sleeper only — public API) ──
            if (!cached && platform === "sleeper" && !_checkedUpcoming[leagueId]) {
              _checkedUpcoming[leagueId] = true;
              try {
                const drafts = await fetch(
                  `https://api.sleeper.app/v1/league/${leagueId}/drafts`
                ).then(r => r.json());
      ...
      items.upcoming.sort((a, b) => a.startTime - b.startTime);
      return items;
    }
    ...
    function _renderPanel(items) {
      ...
      if (items.live.length) {
        ...
        for (const item of items.live) {
          ...
          html += `
            <div class="draft-ticker-row" data-ticker-tid="${_esc(item.tid)}">
              ...
              <div class="draft-ticker-row-info">
                ...
                <div class="draft-ticker-row-meta">${_esc(item.tournamentName)} · ${item.platform}</div>
              </div>
              ${status}
            </div>`;
        }
        ...
        for (const item of items.upcoming) {
          ...
          html += `
            <div class="draft-ticker-row" data-ticker-tid="${_esc(item.tid)}">
              ...
              <div class="draft-ticker-row-info">
                <div class="draft-ticker-row-name">${_esc(item.leagueName)}</div>
                <div class="draft-ticker-row-meta">${_esc(item.tournamentName)}</div>
              </div>
      ...
      // Row click → navigate to that tournament's draft tab
      body.querySelectorAll("[data-ticker-tid]").forEach(row => {
        row.addEventListener("click", () => {
          _closePanel();
          const targetTid = row.dataset.tickerTid;
          // Set draft as the active tab then open the tournament
          _activeUserTab = "draft";
          _openTournamentView(targetTid);
          // Also navigate the main app to the tournaments view
    ...
    // ── Main refresh (called after _loadTournaments and after draft cache update) ──
    async function refresh(allTournaments) {
      const items = await _gatherItems(allTournaments);
      _lastItems = items;
      _updatePill(items);
    ...
    function init(allTournaments) {
      ...
      // Initial data fetch + start 30s poll
      refresh(allTournaments);
      if (_tickerInterval) clearInterval(_tickerInterval);
      _tickerInterval = setInterval(() => refresh(allTournaments), 30000);
    }
